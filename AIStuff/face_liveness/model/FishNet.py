...
class Fish(nn.Module):
    ...
    def _make_score(self, in_ch, out_ch=3, has_pool=False):
        """ make the score for the network """
    ...
    def _make_se_block(self, in_ch, out_ch):
        """ make the se block to boost the accuracy """
        bn = nn.BatchNorm2d(in_ch)
    ...
    def _make_stage(self, is_down_sample, inplanes, outplanes, n_blk, has_trans=True,
        ...
        """ make the stage to describe the intermediate layer """
        sample_block = []
        if has_score:
    ...
    def _make_fish(self, in_planes):
        ...
        cated_planes, _ = [in_planes] * self.depth, []

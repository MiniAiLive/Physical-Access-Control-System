"""
report the detailed parameters and attributes
"""
...
def report_format(collected_nodes):
    ...
    for node in collected_nodes:
        name = node.name
        input_shape = None #' '.join(['{:>3d}'] * len(node.input_shape)).format(*[e for e in node.input_shape])
        output_shape = None #' '.join(['{:>3d}'] * len(node.output_shape)).format(*[e for e in node.output_shape])
        parameter_quantity = node.parameter_quantity
        inference_memory = node.inference_memory
        MAdd = node.MAdd
        Flops = node.Flops
        ConvFlops = node.ConvFlops
        mread, mwrite = None, None #[i for i in node.Memory]
    ...
    df['ConvFlops'] = df['ConvFlops'].apply(lambda x: f'{x:,}')

    summary = str(df) + '\n'
    summary += "=" * len(str(df).split('\n', maxsplit=1)[0])
    summary += '\n'
    summary += f"Total params: {round_value(total_parameters_quantity)}B\n"

    summary += "-" * len(str(df).split('\n', maxsplit=1)[0])
    summary += '\n'
    summary += f"Total memory: {total_memory:.2f}MB\n"
    summary += f"Total MAdd: {round_value(total_operation_quantity)}MAdd\n"
    summary += f"Total Flops: {round_value(total_flops)}Flops\n"
    summary += f"Total Flops(Conv Only): {round_value(total_conv_flops)}Flops\n"

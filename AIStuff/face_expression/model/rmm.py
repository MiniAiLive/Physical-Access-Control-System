...
from src.bioauth_ml.face_expression.model import densenet121
# from src.bioauth_ml.face_utils.utility import download_checkpoint
...
def get_emo_model():
    ...
    emo_model = densenet121()
    if is_cuda:
        emo_model.cuda(0)
    local_checkpoint_path = os.path.join(os.path.dirname(__file__), '../checkpoints/expression.pth')
...
class RMN:
    def __init__(self, face_detector=False):
        if face_detector is True:
            self.face_detector = None #get_ssd_face_detector()
    ...
    def video_demo(self):
        ...
        while True:
            ...
            try:
                ...
                cv2.putText(frame, "press q to exit", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
